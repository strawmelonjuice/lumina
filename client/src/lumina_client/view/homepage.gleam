...
import lumina_client/model_type.{
  type Model, type CachedTimeline, CachedTimeline
}
    ...
    Ok(cached_timeline) -> {
      ...
      html.div([], {
        case posts {
          ...
          _ -> {
            let post_elements = list.map(posts, fn(post_id) {
              html.div([], [
            ...
            case show_load_more {
              True -> list.append(post_elements, [
...
pub fn get_page_posts(timeline: CachedTimeline, page: Int) -> Option(List(String)) {
...
pub fn should_load_more(timeline: CachedTimeline, position: Int, lookahead: Int) -> Bool {
...
pub fn add_page_to_timeline(
  ...
  CachedTimeline(
    ...
    current_page: page,
    has_more: has_more,
    last_updated: 0, // TODO: Add proper timestamp when available
  )
}
...
pub fn timeline_info_string(timeline: CachedTimeline, timeline_name: String) -> String {
  let cached_count = get_cached_posts_count(timeline)
  let highest_page = get_highest_cached_page(timeline)
...
pub fn merge_timelines(old: CachedTimeline, new: CachedTimeline) -> CachedTimeline {
  // Merge pages, preferring new data for conflicts
  let merged_pages = dict.fold(new.pages, old.pages, fn(acc, page, posts) {
    dict.insert(acc, page, posts)
  })

  CachedTimeline(
    pages: merged_pages,
    total_count: new.total_count, // Use new total count

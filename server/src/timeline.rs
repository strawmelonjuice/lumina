...
use crate::{error_elog, user, DbConn};
use redis::Commands;
use uuid::Uuid;
...
pub async fn fetch_timeline_post_ids(event_logger: EventLogger, db: &DbConn, timeline_id: &str) -> Result<Vec<String>, LuminaError> {
    ...
    match db {
        DbConn::PgsqlConnection((client, _pg_config), redis_pool) => {
            ...
            let _: () = redis_conn.incr(format!("timeline_lookup:{}", timeline_id), 1).map_err(LuminaError::Redis)?;
        ...
        DbConn::SqliteConnectionPool(pool, redis_pool) => {
            ...
            if timeline_id == GLOBAL_TIMELINE_ID {
                ...
                let mut stmt = conn.prepare(
                    "SELECT item_id FROM timelines WHERE tlid = ? ORDER BY timestamp DESC"
...
pub async fn fetch_timeline_post_ids_by_timeline_name(event_logger: EventLogger, db: &DbConn, timeline_name: &str, user: user::User) -> Result<(uuid::Uuid,Vec<String>), LuminaError> {
    ...
    if timeline_name == "global" {
        ...
        fetch_timeline_post_ids(event_logger, db, GLOBAL_TIMELINE_ID).await.map(|post_ids| {
            (Uuid::parse_str(GLOBAL_TIMELINE_ID).unwrap(), post_ids)
        })
    } else {

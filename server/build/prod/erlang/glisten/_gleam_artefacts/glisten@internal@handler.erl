...
-type message(LDS) :: {internal, internal_message()} | {user, LDS}.

-type loop_message(LDT) :: {packet, bitstring()} | {custom, LDT}.

-type loop_state(LDU, LDV) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(LDV)),
        glisten@transport:transport(),
        LDU}.

-type connection(LDW) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(LDW))}.

-type next(LDX, LDY) :: {continue,
        LDX,
        gleam@option:option(gleam@erlang@process:selector(LDY))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(LDZ, LEA) :: {handler,
        glisten@socket:socket(),
        fun((LDZ, loop_message(LEA), connection(LEA)) -> next(LDZ, loop_message(LEA))),
        fun((connection(LEA)) -> {LDZ,
            gleam@option:option(gleam@erlang@process:selector(LEA))}),
        gleam@option:option(fun((LDZ) -> nil)),
        glisten@transport:transport(),
        glisten@socket@options:active_state()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(LEN) -> next(LEN, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(LER, LES), gleam@erlang@process:selector(LES)) -> next(LER, LES).
...
?DOC(false).
-spec start(handler(any(), LFH)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(LFH)))} |

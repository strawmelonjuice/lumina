...
-type message(MBG) :: {packet, bitstring()} | {user, MBG}.
...
-type connection(MBH) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(MBH))}.

-opaque next(MBI, MBJ) :: {continue,
        MBI,
        gleam@option:option(gleam@erlang@process:selector(MBJ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(MBK, MBL) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(MBL)) -> {MBK,
            gleam@option:option(gleam@erlang@process:selector(MBL))}),
        fun((MBK, message(MBL), connection(MBL)) -> next(MBK, message(MBL))),
        gleam@option:option(fun((MBK) -> nil)),
        integer(),
        boolean(),
        boolean(),
        ...
        gleam@option:option(gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(MBL))))),
...
-spec continue(MCL) -> next(MCL, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 152).
-spec with_selector(next(MCP, MCQ), gleam@erlang@process:selector(MCQ)) -> next(MCP, MCQ).
...
?DOC(false).
-spec convert_next(next(MDE, MDF)) -> glisten@internal@handler:next(MDE, MDF).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(MDK, MDL), fun((MDL) -> MDO)) -> next(MDK, MDO).
...
-spec map_user_selector(gleam@erlang@process:selector(message(MDR))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(MDR)).
...
-spec convert_loop(
    fun((MDW, message(MDX), connection(MDX)) -> next(MDW, message(MDX)))
) -> fun((MDW, glisten@internal@handler:loop_message(MDX), glisten@internal@handler:connection(MDX)) -> glisten@internal@handler:next(MDW, glisten@internal@handler:loop_message(MDX))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(MEC)) -> {MEE,
        gleam@option:option(gleam@erlang@process:selector(MEC))})
) -> fun((glisten@internal@handler:connection(MEC)) -> {MEE,
    gleam@option:option(gleam@erlang@process:selector(MEC))}).
...
-spec new(
    fun((connection(MEK)) -> {MEM,
        gleam@option:option(gleam@erlang@process:selector(MEK))}),
    fun((MEM, message(MEK), connection(MEK)) -> next(MEM, message(MEK)))
) -> builder(MEM, MEK).
new(On_init, Loop) ->
    {builder,
        loopback,
...
-spec with_close(builder(MET, MEU), fun((MET) -> nil)) -> builder(MET, MEU).
...
-spec with_pool_size(builder(MEZ, MFA), integer()) -> builder(MEZ, MFA).
...
?DOC(false).
-spec with_http2(builder(MFF, MFG)) -> builder(MFF, MFG).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(MFR, MFS)) -> builder(MFR, MFS).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(MFX, MFY), binary(), binary()) -> builder(MFX, MFY).
...
-spec with_connection_factory_name(
    builder(MGQ, MGR),
    gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(MGR))))
) -> builder(MGQ, MGR).
...
-spec bind(builder(MFL, MFM), binary()) -> builder(MFL, MFM).

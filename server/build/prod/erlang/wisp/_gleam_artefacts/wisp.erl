...
-spec method_override(gleam@http@request:request(ACYR)) -> gleam@http@request:request(ACYR).
...
-spec sort_keys(list({binary(), ADAI})) -> list({binary(), ADAI}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, ACZC} | {error, any()}) -> {ok, ACZC} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((ACZQ) -> {ok, ACZR} | {error, any()}),
    binary()
) -> fun((ACZQ) -> {ok, ACZR} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((ACZK, bitstring()) -> {ok, ACZK} |
        {error, gleam@http@response:response(body())}),
    ACZK
) -> {ok, {gleam@option:option(buffered_reader()), integer(), ACZK}} |
    {error, gleam@http@response:response(body())}.

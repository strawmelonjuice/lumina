...
-type transaction_error(ABHN) :: {transaction_query_error, query_error()} |
    {transaction_rolled_back, ABHN}.

-type returned(ABHO) :: {returned, integer(), list(ABHO)}.
...
-opaque 'query'(ABHP) :: {'query',
        binary(),
        list(value()),
        gleam@dynamic@decode:decoder(ABHP),
        integer()}.
...
-spec array(fun((ABIG) -> value()), list(ABIG)) -> value().
...
-spec nullable(fun((ABJB) -> value()), gleam@option:option(ABJB)) -> value().
...
-spec transaction_layer(
    single_connection(),
    fun((connection()) -> {ok, ABIR} | {error, ABIS})
) -> {ok, ABIR} | {error, transaction_error(ABIS)}.
...
-spec transaction(
    connection(),
    fun((connection()) -> {ok, ABIK} | {error, ABIL})
) -> {ok, ABIK} | {error, transaction_error(ABIL)}.
...
-spec returning('query'(any()), gleam@dynamic@decode:decoder(ABJN)) -> 'query'(ABJN).
...
-spec parameter('query'(ABJQ), value()) -> 'query'(ABJQ).
...
-spec timeout('query'(ABJT), integer()) -> 'query'(ABJT).
...
-spec execute('query'(ABJW), connection()) -> {ok, returned(ABJW)} |
    {error, query_error()}.
execute(Query, Pool) ->

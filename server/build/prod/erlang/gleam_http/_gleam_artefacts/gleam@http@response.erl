...
-type response(EUT) :: {response, integer(), list({binary(), binary()}), EUT}.
...
-spec set_header(response(EVI), binary(), binary()) -> response(EVI).
...
-spec prepend_header(response(EVL), binary(), binary()) -> response(EVL).
...
-spec set_body(response(any()), EVQ) -> response(EVQ).
...
-spec try_map(response(EUU), fun((EUU) -> {ok, EUW} | {error, EUX})) -> {ok,
        response(EUW)} |
    {error, EUX}.
...
-spec map(response(EVS), fun((EVS) -> EVU)) -> response(EVU).
...
-spec expire_cookie(response(EWD), binary(), gleam@http@cookie:attributes()) -> response(EWD).

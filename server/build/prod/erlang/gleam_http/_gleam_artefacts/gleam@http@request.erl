...
-spec set_header(request(EKZ), binary(), binary()) -> request(EKZ).
...
-spec prepend_header(request(ELC), binary(), binary()) -> request(ELC).
...
-spec set_body(request(any()), ELH) -> request(ELH).
set_body(Req, Body) ->
    {request,
...
-spec map(request(ELJ), fun((ELJ) -> ELL)) -> request(ELL).
...
-spec set_query(request(ELV), list({binary(), binary()})) -> request(ELV).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(ELZ), gleam@http:method()) -> request(ELZ).
...
-spec set_scheme(request(EMG), gleam@http:scheme()) -> request(EMG).
...
-spec set_host(request(EMJ), binary()) -> request(EMJ).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(EMM), integer()) -> request(EMM).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(EMP), binary()) -> request(EMP).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(EMS), binary(), binary()) -> request(EMS).
...
-spec remove_cookie(request(EMY), binary()) -> request(EMY).

...
-spec map_response_body(
    fun((EYP) -> gleam@http@response:response(EYQ)),
    fun((EYQ) -> EYS)
) -> fun((EYP) -> gleam@http@response:response(EYS)).
...
-spec prepend_response_header(
    fun((EYU) -> gleam@http@response:response(EYV)),
    binary(),
    binary()
) -> fun((EYU) -> gleam@http@response:response(EYV)).
...
-spec ensure_post(gleam@http@request:request(EYY)) -> {ok,
        gleam@http@request:request(EYY)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(EZH)) -> EZJ)) -> fun((gleam@http@request:request(EZH)) -> EZJ).

...
-type array(QUS) :: any() | {gleam_phantom, QUS}.
...
-spec from_list(list(QUV)) -> array(QUV).
from_list(List) ->
    erlang:list_to_tuple(List).

-file("src/glearray.gleam", 44).
?DOC(" Converts an array to a list.\n").
-spec to_list(array(QUY)) -> list(QUY).
to_list(Array) ->
...
-spec get(array(QVD), integer()) -> {ok, QVD} | {error, nil}.
get(Array, Index) ->
...
-spec get_or_default(array(QVH), integer(), QVH) -> QVH.
...
-spec copy_set(array(QVL), integer(), QVL) -> {ok, array(QVL)} | {error, nil}.
...
-spec copy_push(array(QVV), QVV) -> array(QVV).
...
-spec copy_insert(array(QVY), integer(), QVY) -> {ok, array(QVY)} | {error, nil}.

...
-type action(IUM) :: stop | {continue, IUM, fun(() -> action(IUM))}.

-opaque yielder(IUN) :: {yielder, fun(() -> action(IUN))}.

-type step(IUO, IUP) :: {next, IUO, IUP} | done.

-type chunk(IUQ, IUR) :: {another_by,
        list(IUQ),
        IUR,
        IUQ,
        fun(() -> action(IUQ))} |
...
-type sized_chunk(IUS) :: {another, list(IUS), fun(() -> action(IUS))} |
    {last, list(IUS)} |
    no_more.
...
-spec unfold_loop(IVA, fun((IVA) -> step(IVB, IVA))) -> fun(() -> action(IVB)).
...
-spec unfold(IUV, fun((IUV) -> step(IUW, IUV))) -> yielder(IUW).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> IVF)) -> yielder(IVF).
repeatedly(F) ->
...
-spec from_list(list(IVJ)) -> yielder(IVJ).
from_list(List) ->
...
-spec transform(yielder(IVT), IVV, fun((IVV, IVT) -> step(IVW, IVV))) -> yielder(IVW).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(IWD)), fun((IWF, IWD) -> IWF), IWF) -> IWF.
...
-spec fold(yielder(IWA), IWC, fun((IWC, IWA) -> IWC)) -> IWC.
...
-spec to_list(yielder(IWI)) -> list(IWI).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(IWL)) -> step(IWL, yielder(IWL)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(IWT)), integer()) -> fun(() -> action(IWT)).
...
-spec take(yielder(IWQ), integer()) -> yielder(IWQ).
...
-spec drop_loop(fun(() -> action(IWZ)), integer()) -> action(IWZ).
...
-spec map(yielder(IXC), fun((IXC) -> IXE)) -> yielder(IXE).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(IXK), yielder(IXM), fun((IXK, IXM) -> IXO)) -> yielder(IXO).
...
-spec append_loop(fun(() -> action(IYA)), fun(() -> action(IYA))) -> action(IYA).
...
-spec append(yielder(IXW), yielder(IXW)) -> yielder(IXW).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(IYI)))) -> action(IYI).
...
-spec flatten(yielder(yielder(IYE))) -> yielder(IYE).
flatten(Yielder) ->
...
-spec concat(list(yielder(IYM))) -> yielder(IYM).
concat(Yielders) ->
...
-spec flat_map(yielder(IYQ), fun((IYQ) -> yielder(IYS))) -> yielder(IYS).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(IYY)), fun((IYY) -> boolean())) -> action(IYY).
...
-spec filter(yielder(IYV), fun((IYV) -> boolean())) -> yielder(IYV).
...
-spec filter_map(yielder(IZB), fun((IZB) -> {ok, IZD} | {error, any()})) -> yielder(IZD).
...
-spec cycle(yielder(IZP)) -> yielder(IZP).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(IZX)), fun((IZX) -> boolean())) -> {ok, IZX} |
    {error, nil}.
...
-spec find(yielder(IZT), fun((IZT) -> boolean())) -> {ok, IZT} | {error, nil}.
...
-spec find_map(yielder(JAB), fun((JAB) -> {ok, JAD} | {error, any()})) -> {ok,
        JAD} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(JAU)), integer()) -> fun(() -> action({JAU,
    integer()})).
...
-spec index(yielder(JAR)) -> yielder({JAR, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(JAX, fun((JAX) -> JAX)) -> yielder(JAX).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(JBC)), fun((JBC) -> boolean())) -> fun(() -> action(JBC)).
...
-spec take_while(yielder(JAZ), fun((JAZ) -> boolean())) -> yielder(JAZ).
...
-spec drop_while_loop(fun(() -> action(JBI)), fun((JBI) -> boolean())) -> action(JBI).
...
-spec drop_while(yielder(JBF), fun((JBF) -> boolean())) -> yielder(JBF).
...
-spec scan_loop(fun(() -> action(JBP)), fun((JBR, JBP) -> JBR), JBR) -> fun(() -> action(JBR)).
...
-spec scan(yielder(JBL), JBN, fun((JBN, JBL) -> JBN)) -> yielder(JBN).
...
-spec zip_loop(fun(() -> action(JBY)), fun(() -> action(JCA))) -> fun(() -> action({JBY,
    JCA})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(JBT), yielder(JBV)) -> yielder({JBT, JBV}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(JCN)), fun((JCN) -> JCP), JCP, list(JCN)) -> chunk(JCN, JCP).
...
-spec chunk_loop(fun(() -> action(JCI)), fun((JCI) -> JCK), JCK, JCI) -> action(list(JCI)).
...
-spec chunk(yielder(JCD), fun((JCD) -> any())) -> yielder(list(JCD)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(JDB)), integer(), list(JDB)) -> sized_chunk(JDB).
...
-spec sized_chunk_loop(fun(() -> action(JCX)), integer()) -> fun(() -> action(list(JCX))).
...
-spec sized_chunk(yielder(JCT), integer()) -> yielder(list(JCT)).
...
-spec intersperse_loop(fun(() -> action(JDI)), JDI) -> action(JDI).
...
-spec intersperse(yielder(JDF), JDF) -> yielder(JDF).
...
-spec any_loop(fun(() -> action(JDN)), fun((JDN) -> boolean())) -> boolean().
...
-spec any(yielder(JDL), fun((JDL) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(JDR)), fun((JDR) -> boolean())) -> boolean().
...
-spec all(yielder(JDP), fun((JDP) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(JEH) -> fun((gleam@option:option(list(JEH))) -> list(JEH)).
...
-spec group_updater(fun((JDZ) -> JEA)) -> fun((gleam@dict:dict(JEA, list(JDZ)), JDZ) -> gleam@dict:dict(JEA, list(JDZ))).
group_updater(F) ->
...
-spec group(yielder(JDT), fun((JDT) -> JDV)) -> gleam@dict:dict(JDV, list(JDT)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(JEL), fun((JEL, JEL) -> JEL)) -> {ok, JEL} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(JEP)) -> {ok, JEP} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(JEX) -> yielder(JEX).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(JFD)), fun(() -> action(JFD))) -> action(JFD).
...
-spec interleave(yielder(JEZ), yielder(JEZ)) -> yielder(JEZ).
...
-spec fold_until_loop(
    fun(() -> action(JFL)),
    fun((JFN, JFL) -> gleam@list:continue_or_stop(JFN)),
    JFN
) -> JFN.
...
-spec fold_until(
    yielder(JFH),
    JFJ,
    fun((JFJ, JFH) -> gleam@list:continue_or_stop(JFJ))
) -> JFJ.
...
-spec try_fold(yielder(JFP), JFR, fun((JFR, JFP) -> {ok, JFR} | {error, JFS})) -> {ok,
        JFR} |
    {error, JFS}.
...
-spec first(yielder(JGF)) -> {ok, JGF} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(JGJ), integer()) -> {ok, JGJ} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(JGR), fun((JGR) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(JGU, fun(() -> yielder(JGU))) -> yielder(JGU).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(JGX), JGX) -> yielder(JGX).

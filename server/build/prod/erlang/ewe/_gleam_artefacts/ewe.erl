...
-opaque chunked_next(PLJ) :: {chunked_continue, PLJ} |
    chunked_stop |
    {chunked_abnormal_stop, binary()}.

-opaque websocket_next(PLK, PLL) :: {websocket_continue,
        PLK,
        gleam@option:option(gleam@erlang@process:selector(PLL))} |
    websocket_normal_stop |
    {websocket_abnormal_stop, binary()}.

-type websocket_message(PLM) :: {text, binary()} |
    {binary, bitstring()} |
    {user, PLM}.
...
-opaque s_s_e_next(PLN) :: {s_s_e_continue, PLN} |
    s_s_e_normal_stop |
...
-spec chunked_continue(PMM) -> chunked_next(PMM).
...
-spec to_internal_chunked_next(chunked_next(PMS)) -> ewe@internal@stream@chunked:chunked_next(PMS).
...
-spec chunked_body(
    ...
    fun((gleam@erlang@process:subject(PMX)) -> PMZ),
    fun((ewe@internal@stream@chunked:chunked_body(), PMZ, PMX) -> chunked_next(PMZ)),
    fun((ewe@internal@stream@chunked:chunked_body(), PMZ) -> nil)
) -> gleam@http@response:response(response_body()).
...
-spec websocket_continue(PND) -> websocket_next(PND, any()).
...
-spec websocket_continue_with_selector(PNH, gleam@erlang@process:selector(PNI)) -> websocket_next(PNH, PNI).
...
-spec to_websocket_next(ewe@internal@stream@websocket:websocket_next(PNU, PNV)) -> websocket_next(PNU, PNV).
...
-spec to_internal_websocket_next(websocket_next(POA, POB)) -> ewe@internal@stream@websocket:websocket_next(POA, POB).
...
-spec transform_websocket_message(
    ewe@internal@stream@websocket:websocket_message(POG)
) -> {ok, websocket_message(POG)} | {error, nil}.
...
-spec upgrade_websocket(
    gleam@http@request:request(ewe@internal@http1:connection()),
    fun((ewe@internal@stream@websocket:websocket_connection(), gleam@erlang@process:selector(POL)) -> {PON,
        gleam@erlang@process:selector(POL)}),
    fun((ewe@internal@stream@websocket:websocket_connection(), PON, websocket_message(POL)) -> websocket_next(PON, POL)),
    fun((ewe@internal@stream@websocket:websocket_connection(), PON) -> nil)
) -> gleam@http@response:response(response_body()).
...
-spec sse_continue(PPA) -> s_s_e_next(PPA).
...
-spec sse(
    gleam@http@request:request(ewe@internal@http1:connection()),
    fun((gleam@erlang@process:subject(PPJ)) -> PPL),
    fun((ewe@internal@stream@sse:s_s_e_connection(), PPL, PPJ) -> s_s_e_next(PPL)),
    fun((ewe@internal@stream@sse:s_s_e_connection(), PPL) -> nil)
) -> gleam@http@response:response(response_body()).
...
-spec to_internal_sse_next(s_s_e_next(PPG)) -> ewe@internal@stream@sse:s_s_e_next(PPG).

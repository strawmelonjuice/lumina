...
-spec with_context(list({binary(), binary()}), fun(() -> AEGQ)) -> AEGQ.
...
-spec tap_info(AEGU, binary(), list({binary(), binary()})) -> AEGU.
...
-spec tap_debug(AEGW, binary(), list({binary(), binary()})) -> AEGW.
...
-spec tap_warning(AEGY, binary(), list({binary(), binary()})) -> AEGY.
...
-spec tap_error(AEHA, binary(), list({binary(), binary()})) -> AEHA.
...
-spec time(binary(), fun(() -> AEHJ)) -> AEHJ.
time(Label, Body) ->

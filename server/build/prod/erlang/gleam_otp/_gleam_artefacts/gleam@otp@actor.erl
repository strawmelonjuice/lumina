...
-type message(FBR) :: {message, FBR} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FBS, FBT) :: {continue,
        FBS,
        gleam@option:option(gleam@erlang@process:selector(FBT))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FBU, FBV) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FBU,
        gleam@erlang@process:selector(message(FBV)),
        gleam@otp@system:debug_state(),
        fun((FBU, FBV) -> next(FBU, FBV))}.

-type started(FBW) :: {started, gleam@erlang@process:pid_(), FBW}.

-opaque initialised(FBX, FBY, FBZ) :: {initialised,
        FBX,
        gleam@option:option(gleam@erlang@process:selector(FBY)),
        FBZ}.

-opaque builder(FCA, FCB, FCC) :: {builder,
        fun((gleam@erlang@process:subject(FCB)) -> {ok,
                initialised(FCA, FCB, FCC)} |
            {error, binary()}),
        integer(),
        fun((FCA, FCB) -> next(FCA, FCB)),
        gleam@option:option(gleam@erlang@process:name(FCB))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FCD) :: {ack, {ok, FCD} | {error, binary()}} |
...
-spec continue(FCI) -> next(FCI, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FCU, FCV), gleam@erlang@process:selector(FCV)) -> next(FCU, FCV).
...
-spec initialised(FDB) -> initialised(FDB, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FDG, any(), FDI),
    gleam@erlang@process:selector(FDM)
) -> initialised(FDG, FDM, FDI).
...
-spec returning(initialised(FDR, FDS, any()), FDX) -> initialised(FDR, FDS, FDX).
...
-spec new(FEB) -> builder(FEB, FEC, gleam@erlang@process:subject(FEC)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FEH)) -> {ok, initialised(FEJ, FEH, FEK)} |
        {error, binary()})
) -> builder(FEJ, FEH, FEK).
...
-spec on_message(builder(FET, FEU, FEV), fun((FET, FEU) -> next(FET, FEU))) -> builder(FET, FEU, FEV).
...
-spec named(builder(FFE, FFF, FFG), gleam@erlang@process:name(FFF)) -> builder(FFE, FFF, FFG).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FFT))) -> gleam@erlang@process:selector(message(FFT)).
...
-spec receive_message(self(any(), FFP)) -> message(FFP).
...
-spec initialise_actor(
    builder(any(), any(), FGM),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FGM} | {error, binary()})
...
-spec start(builder(any(), any(), FGZ)) -> {ok, started(FGZ)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(FHG), FHG) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FHI),
    integer(),
    fun((gleam@erlang@process:subject(FHK)) -> FHI)
) -> FHK.

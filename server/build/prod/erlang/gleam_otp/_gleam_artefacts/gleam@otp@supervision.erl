...
-type child_specification(FUW) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(FUW)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(FUX)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FUX).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(FVC)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FVC).
supervisor(Start) ->
...
-spec significant(child_specification(FVH), boolean()) -> child_specification(FVH).
...
-spec timeout(child_specification(FVK), integer()) -> child_specification(FVK).
timeout(Child, Ms) ->
...
-spec restart(child_specification(FVN), restart()) -> child_specification(FVN).
...
-spec map_data(child_specification(FVQ), fun((FVQ) -> FVS)) -> child_specification(FVS).

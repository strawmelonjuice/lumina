...
-opaque supervisor(FXK, FXL) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(FXK, FXL))}.

-type message(FXM, FXN) :: any() | {gleam_phantom, FXM, FXN}.

-opaque builder(FXO, FXP) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((FXO) -> {ok, gleam@otp@actor:started(FXP)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(FXO, FXP)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(FXQ, FXR) :: {local,
        gleam@erlang@process:name(message(FXQ, FXR))}.
...
-type erlang_child_spec_property(FXT, FXU) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((FXT) -> {ok, gleam@otp@actor:started(FXU)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(FXV, FXW))) -> supervisor(FXV, FXW).
get_by_name(Name) ->
...
-spec named(builder(FYM, FYN), gleam@erlang@process:name(message(FYM, FYN))) -> builder(FYM, FYN).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(FYV, FYW), integer(), integer()) -> builder(FYV, FYW).
...
-spec timeout(builder(FZB, FZC), integer()) -> builder(FZB, FZC).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(FZH, FZI), gleam@otp@supervision:restart()) -> builder(FZH, FZI).
...
-spec start(builder(FZN, FZO)) -> {ok,
        gleam@otp@actor:started(supervisor(FZN, FZO))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(GAM, GAN)) -> gleam@otp@supervision:child_specification(supervisor(GAM, GAN)).
supervised(Builder) ->
...
-spec start_child(supervisor(GAT, GAU), GAT) -> {ok,
        gleam@otp@actor:started(GAU)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((GBR) -> {ok, gleam@otp@actor:started(GBS)} |
        {error, gleam@otp@actor:start_error()}),
    GBR
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), GBS, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((FYC) -> {ok, gleam@otp@actor:started(FYD)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FYC, FYD).

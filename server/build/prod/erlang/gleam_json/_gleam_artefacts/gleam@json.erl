...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(QGD)) -> {ok, QGD} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(QFX)) -> {ok, QFX} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(QFT)) -> {ok, QFT} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(QGJ), fun((QGJ) -> json())) -> json().
...
-spec array(list(QGN), fun((QGN) -> json())) -> json().

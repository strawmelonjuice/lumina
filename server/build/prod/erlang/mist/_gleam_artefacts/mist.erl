...
-opaque next(VFP, VFQ) :: {continue,
        VFP,
        gleam@option:option(gleam@erlang@process:selector(VFQ))} |
    normal_stop |
...
-opaque builder(VFR, VFS) :: {builder,
        integer(),
        fun((gleam@http@request:request(VFR)) -> gleam@http@response:response(VFS)),
...
-type websocket_message(VFT) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, VFT}.
...
-type chunk_next(VFU) :: {chunk_continue, VFU} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(VFV) -> next(VFV, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(VFZ, VGA), gleam@erlang@process:selector(VGA)) -> next(VFZ, VGA).
...
-spec convert_next(next(VGO, VGP)) -> mist@internal@next:next(VGO, VGP).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(VHQ)) -> gleam@http@response:response(VHS))
) -> builder(VHQ, VHS).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(VHW, VHX), integer()) -> builder(VHW, VHX).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), VIC),
    integer(),
    gleam@http@response:response(VIC)
) -> builder(mist@internal@http:connection(), VIC).
...
-spec bind(builder(VIO, VIP), binary()) -> builder(VIO, VIP).
...
-spec with_ipv6(builder(VIU, VIV)) -> builder(VIU, VIV).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(VJA, VJB), binary(), binary()) -> builder(VJA, VJB).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(VJQ)
) -> {ok, websocket_message(VJQ)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((VJW, websocket_message(VJX), mist@internal@websocket:websocket_connection()) -> next(VJW, VJX)),
    fun((mist@internal@websocket:websocket_connection()) -> {VJW,
        gleam@option:option(gleam@erlang@process:selector(VJX))}),
    fun((VJW) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(VKL)) -> VKN),
    fun((VKN, VKL, s_s_e_connection()) -> gleam@otp@actor:next(VKN, VKL))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(VLD) -> chunk_next(VLD).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(VKW)) -> VKY),
    fun((VKY, VKW, mist@internal@http:connection()) -> chunk_next(VKY))
) -> gleam@http@response:response(response_data()).

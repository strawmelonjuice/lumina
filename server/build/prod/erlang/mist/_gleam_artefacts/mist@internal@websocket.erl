...
-type valid_message(URM) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, URM}.

-type websocket_message(URN) :: {valid, valid_message(URN)} | invalid.
...
-type handler_message(URO) :: {internal, gramps@websocket:frame()} | {user, URO}.

-type websocket_state(URP) :: {websocket_state,
        bitstring(),
        URP,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(UST))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(UST))).
...
-spec apply_frames(
    ...
    fun((USJ, handler_message(USK), websocket_connection()) -> mist@internal@next:next(USJ, USK)),
    websocket_connection(),
    mist@internal@next:next(USJ, websocket_message(USK)),
    fun((USJ) -> nil)
) -> mist@internal@next:next(USJ, websocket_message(USK)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {URY,
        gleam@option:option(gleam@erlang@process:selector(URZ))}),
    fun((URY) -> nil),
    fun((URY, handler_message(URZ), websocket_connection()) -> mist@internal@next:next(URY, URZ)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())

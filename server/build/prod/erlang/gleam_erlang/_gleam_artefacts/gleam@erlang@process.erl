...
-opaque subject(DOX) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DOX)}.

-type name(DOY) :: any() | {gleam_phantom, DOY}.

-type do_not_leak() :: any().

-type selector(DOZ) :: any() | {gleam_phantom, DOZ}.
...
-spec 'receive'(subject(DPX), integer()) -> {ok, DPX} | {error, nil}.
...
-spec receive_forever(subject(DQF)) -> DQF.
...
-spec selector_receive(selector(DQJ), integer()) -> {ok, DQJ} | {error, nil}.
...
-spec selector_receive_forever(selector(DQN)) -> DQN.
...
-spec map_selector(selector(DQP), fun((DQP) -> DQR)) -> selector(DQR).
map_selector(A, B) ->
...
-spec merge_selector(selector(DQT), selector(DQT)) -> selector(DQT).
merge_selector(A, B) ->
...
-spec select_map(selector(DRE), subject(DRG), fun((DRG) -> DRE)) -> selector(DRE).
...
-spec select(selector(DRA), subject(DRA)) -> selector(DRA).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DRO)
) -> selector(DRO).
...
-spec select_other(selector(DRS), fun((gleam@dynamic:dynamic_()) -> DRS)) -> selector(DRS).
...
-spec deselect(selector(DRJ), subject(any())) -> selector(DRJ).
...
-spec select_specific_monitor(selector(DSE), monitor(), fun((down()) -> DSE)) -> selector(DSE).
...
-spec select_monitors(selector(DSH), fun((down()) -> DSH)) -> selector(DSH).
...
-spec select_trapped_exits(selector(DQX), fun((exit_message()) -> DQX)) -> selector(DQX).
...
-spec deselect_specific_monitor(selector(DSK), monitor()) -> selector(DSK).
...
-spec send_after(subject(DTG), integer(), DTG) -> timer().
...
-spec perform_call(
    subject(DSN),
    fun((subject(DSP)) -> DSN),
    fun((selector(DSP)) -> {ok, DSP} | {error, nil})
) -> DSP.
...
-spec call(subject(DSU), integer(), fun((subject(DSW)) -> DSU)) -> DSW.
...
-spec call_forever(subject(DSY), fun((subject(DTA)) -> DSY)) -> DTA.

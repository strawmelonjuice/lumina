...
-spec with_connection(binary(), fun((connection()) -> ACVI)) -> ACVI.
...
-spec nullable(fun((ACVY) -> value()), gleam@option:option(ACVY)) -> value().
...
-spec 'query'(
    ...
    connection(),
    list(value()),
    gleam@dynamic@decode:decoder(ACVM)
) -> {ok, list(ACVM)} | {error, error()}.

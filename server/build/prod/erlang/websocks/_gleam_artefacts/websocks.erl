...
-type resolve_next(GMX) :: {continue, GMX} | {stop, GMX}.
...
-spec resolve_and_handle_single_frame(
    ...
    context(),
    GOH,
    fun((GOH, context(), frame()) -> resolve_next(GOH))
) -> {ok, {resolve_next(GOH), context()}} | {error, resolve_error()}.
...
-spec do_process_incoming_frames(
    ...
    context(),
    GOE,
    fun((GOE, context(), frame()) -> resolve_next(GOE))
) -> {ok, {GOE, context()}} | {error, process_error()}.
...
-spec process_incoming_frames(
    ...
    context(),
    GOB,
    fun((GOB, context(), frame()) -> resolve_next(GOB))
) -> {ok, {GOB, context()}} | {error, process_error()}.

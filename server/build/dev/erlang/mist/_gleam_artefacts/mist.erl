...
-opaque next(VQP, VQQ) :: {continue,
        VQP,
        gleam@option:option(gleam@erlang@process:selector(VQQ))} |
    normal_stop |
...
-opaque builder(VQR, VQS) :: {builder,
        integer(),
        fun((gleam@http@request:request(VQR)) -> gleam@http@response:response(VQS)),
...
-type websocket_message(VQT) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, VQT}.
...
-type chunk_next(VQU) :: {chunk_continue, VQU} |
    chunk_stop |
    {chunk_abort, binary()}.

-file("src/mist.gleam", 53).
-spec continue(VQV) -> next(VQV, any()).
continue(State) ->
    {continue, State, none}.

-file("src/mist.gleam", 57).
-spec with_selector(next(VQZ, VRA), gleam@erlang@process:selector(VRA)) -> next(VQZ, VRA).
...
-spec convert_next(next(VRO, VRP)) -> mist@internal@next:next(VRO, VRP).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(VSQ)) -> gleam@http@response:response(VSS))
) -> builder(VSQ, VSS).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(VSW, VSX), integer()) -> builder(VSW, VSX).
port(Builder, Port) ->
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), VTC),
    integer(),
    gleam@http@response:response(VTC)
) -> builder(mist@internal@http:connection(), VTC).
...
-spec bind(builder(VTO, VTP), binary()) -> builder(VTO, VTP).
...
-spec with_ipv6(builder(VTU, VTV)) -> builder(VTU, VTV).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(VUA, VUB), binary(), binary()) -> builder(VUA, VUB).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(VUQ)
) -> {ok, websocket_message(VUQ)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((VUW, websocket_message(VUX), mist@internal@websocket:websocket_connection()) -> next(VUW, VUX)),
    fun((mist@internal@websocket:websocket_connection()) -> {VUW,
        gleam@option:option(gleam@erlang@process:selector(VUX))}),
    fun((VUW) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(VVL)) -> VVN),
    fun((VVN, VVL, s_s_e_connection()) -> gleam@otp@actor:next(VVN, VVL))
) -> gleam@http@response:response(response_data()).
...
-spec chunk_continue(VWD) -> chunk_next(VWD).
...
-spec chunked(
    gleam@http@request:request(mist@internal@http:connection()),
    gleam@http@response:response(any()),
    fun((gleam@erlang@process:subject(VVW)) -> VVY),
    fun((VVY, VVW, mist@internal@http:connection()) -> chunk_next(VVY))
) -> gleam@http@response:response(response_data()).

...
-type valid_message(VCM) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, VCM}.

-type websocket_message(VCN) :: {valid, valid_message(VCN)} | invalid.
...
-type handler_message(VCO) :: {internal, gramps@websocket:frame()} | {user, VCO}.

-type websocket_state(VCP) :: {websocket_state,
        bitstring(),
        VCP,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(VDT))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(VDT))).
...
-spec apply_frames(
    ...
    fun((VDJ, handler_message(VDK), websocket_connection()) -> mist@internal@next:next(VDJ, VDK)),
    websocket_connection(),
    mist@internal@next:next(VDJ, websocket_message(VDK)),
    fun((VDJ) -> nil)
) -> mist@internal@next:next(VDJ, websocket_message(VDK)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {VCY,
        gleam@option:option(gleam@erlang@process:selector(VCZ))}),
    fun((VCY) -> nil),
    fun((VCY, handler_message(VCZ), websocket_connection()) -> mist@internal@next:next(VCY, VCZ)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())

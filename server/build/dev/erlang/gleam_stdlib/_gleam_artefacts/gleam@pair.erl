...
-spec map_first({CNH, CNI}, fun((CNH) -> CNJ)) -> {CNJ, CNI}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CNK, CNL}, fun((CNL) -> CNM)) -> {CNK, CNM}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.

...
-spec map({ok, CNY} | {error, CNZ}, fun((CNY) -> COC)) -> {ok, COC} |
    {error, CNZ}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, COF} | {error, COG}, fun((COG) -> COJ)) -> {ok, COF} |
    {error, COJ}.
...
-spec flatten({ok, {ok, COM} | {error, CON}} | {error, CON}) -> {ok, COM} |
    {error, CON}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, COU} | {error, COV}, fun((COU) -> {ok, COY} | {error, COV})) -> {ok,
        COY} |
    {error, COV}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, CPD} | {error, any()}, CPD) -> CPD.
...
-spec lazy_unwrap({ok, CPH} | {error, any()}, fun(() -> CPH)) -> CPH.
...
-spec unwrap_error({ok, any()} | {error, CPM}, CPM) -> CPM.
...
-spec 'or'({ok, CPP} | {error, CPQ}, {ok, CPP} | {error, CPQ}) -> {ok, CPP} |
    {error, CPQ}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CPX} | {error, CPY}, fun(() -> {ok, CPX} | {error, CPY})) -> {ok,
        CPX} |
    {error, CPY}.
...
-spec all(list({ok, CQF} | {error, CQG})) -> {ok, list(CQF)} | {error, CQG}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 307).
-spec partition_loop(list({ok, CQU} | {error, CQV}), list(CQU), list(CQV)) -> {list(CQU),
    list(CQV)}.
...
-spec partition(list({ok, CQN} | {error, CQO})) -> {list(CQN), list(CQO)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CRD}, CRG) -> {ok, CRG} | {error, CRD}.
...
-spec replace_error({ok, CRJ} | {error, any()}, CRN) -> {ok, CRJ} | {error, CRN}.
...
-spec values(list({ok, CRQ} | {error, any()})) -> list(CRQ).
values(Results) ->

...
-type message(FCK) :: {message, FCK} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FCL, FCM) :: {continue,
        FCL,
        gleam@option:option(gleam@erlang@process:selector(FCM))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FCN, FCO) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FCN,
        gleam@erlang@process:selector(message(FCO)),
        gleam@otp@system:debug_state(),
        fun((FCN, FCO) -> next(FCN, FCO))}.

-type started(FCP) :: {started, gleam@erlang@process:pid_(), FCP}.

-opaque initialised(FCQ, FCR, FCS) :: {initialised,
        FCQ,
        gleam@option:option(gleam@erlang@process:selector(FCR)),
        FCS}.

-opaque builder(FCT, FCU, FCV) :: {builder,
        fun((gleam@erlang@process:subject(FCU)) -> {ok,
                initialised(FCT, FCU, FCV)} |
            {error, binary()}),
        integer(),
        fun((FCT, FCU) -> next(FCT, FCU)),
        gleam@option:option(gleam@erlang@process:name(FCU))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FCW) :: {ack, {ok, FCW} | {error, binary()}} |
...
-spec continue(FDB) -> next(FDB, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FDN, FDO), gleam@erlang@process:selector(FDO)) -> next(FDN, FDO).
...
-spec initialised(FDU) -> initialised(FDU, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FDZ, any(), FEB),
    gleam@erlang@process:selector(FEF)
) -> initialised(FDZ, FEF, FEB).
...
-spec returning(initialised(FEK, FEL, any()), FEQ) -> initialised(FEK, FEL, FEQ).
...
-spec new(FEU) -> builder(FEU, FEV, gleam@erlang@process:subject(FEV)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FFA)) -> {ok, initialised(FFC, FFA, FFD)} |
        {error, binary()})
) -> builder(FFC, FFA, FFD).
...
-spec on_message(builder(FFM, FFN, FFO), fun((FFM, FFN) -> next(FFM, FFN))) -> builder(FFM, FFN, FFO).
...
-spec named(builder(FFX, FFY, FFZ), gleam@erlang@process:name(FFY)) -> builder(FFX, FFY, FFZ).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FGM))) -> gleam@erlang@process:selector(message(FGM)).
...
-spec receive_message(self(any(), FGI)) -> message(FGI).
...
-spec initialise_actor(
    builder(any(), any(), FHF),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FHF} | {error, binary()})
...
-spec start(builder(any(), any(), FHS)) -> {ok, started(FHS)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(FHZ), FHZ) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FIB),
    integer(),
    fun((gleam@erlang@process:subject(FID)) -> FIB)
) -> FID.

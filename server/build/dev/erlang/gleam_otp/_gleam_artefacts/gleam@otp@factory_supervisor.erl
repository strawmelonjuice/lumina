...
-opaque supervisor(FYD, FYE) :: {supervisor, gleam@erlang@process:pid_()} |
    {named_supervisor, gleam@erlang@process:name(message(FYD, FYE))}.

-type message(FYF, FYG) :: any() | {gleam_phantom, FYF, FYG}.

-opaque builder(FYH, FYI) :: {builder,
        gleam@otp@supervision:child_type(),
        fun((FYH) -> {ok, gleam@otp@actor:started(FYI)} |
            {error, gleam@otp@actor:start_error()}),
        gleam@otp@supervision:restart(),
        integer(),
        integer(),
        gleam@option:option(gleam@erlang@process:name(message(FYH, FYI)))}.

-type erlang_start_flags() :: any().

-type erlang_supervisor_name(FYJ, FYK) :: {local,
        gleam@erlang@process:name(message(FYJ, FYK))}.

-type strategy() :: simple_one_for_one.

-type erlang_start_flag(FYL) :: {strategy, strategy()} |
    {intensity, integer()} |
    {period, integer()} |
    {gleam_phantom, FYL}.
...
-type erlang_child_spec_property(FYM, FYN) :: {id, integer()} |
    {start,
        {gleam@erlang@atom:atom_(),
            gleam@erlang@atom:atom_(),
            list(fun((FYM) -> {ok, gleam@otp@actor:started(FYN)} |
                {error, gleam@otp@actor:start_error()}))}} |
...
-spec get_by_name(gleam@erlang@process:name(message(FYO, FYP))) -> supervisor(FYO, FYP).
get_by_name(Name) ->
...
-spec named(builder(FZF, FZG), gleam@erlang@process:name(message(FZF, FZG))) -> builder(FZF, FZG).
named(Builder, Name) ->
    {builder,
...
-spec restart_tolerance(builder(FZO, FZP), integer(), integer()) -> builder(FZO, FZP).
...
-spec timeout(builder(FZU, FZV), integer()) -> builder(FZU, FZV).
timeout(Builder, Ms) ->
...
-spec restart_strategy(builder(GAA, GAB), gleam@otp@supervision:restart()) -> builder(GAA, GAB).
...
-spec start(builder(GAG, GAH)) -> {ok,
        gleam@otp@actor:started(supervisor(GAG, GAH))} |
    {error, gleam@otp@actor:start_error()}.
start(Builder) ->
...
-spec supervised(builder(GBF, GBG)) -> gleam@otp@supervision:child_specification(supervisor(GBF, GBG)).
supervised(Builder) ->
...
-spec start_child(supervisor(GBM, GBN), GBM) -> {ok,
        gleam@otp@actor:started(GBN)} |
    {error, gleam@otp@actor:start_error()}.
...
-spec start_child_callback(
    fun((GCK) -> {ok, gleam@otp@actor:started(GCL)} |
        {error, gleam@otp@actor:start_error()}),
    GCK
) -> gleam@otp@internal@result2:result2(gleam@erlang@process:pid_(), GCL, gleam@otp@actor:start_error()).
...
-spec worker_child(
    fun((FYV) -> {ok, gleam@otp@actor:started(FYW)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FYV, FYW).
...
-spec supervisor_child(
    fun((FZA) -> {ok, gleam@otp@actor:started(FZB)} |
        {error, gleam@otp@actor:start_error()})
) -> builder(FZA, FZB).

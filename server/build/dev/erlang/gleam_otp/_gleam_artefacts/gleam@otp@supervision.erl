...
-type child_specification(FVP) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(FVP)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(FVQ)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FVQ).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(FVV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(FVV).
supervisor(Start) ->
...
-spec significant(child_specification(FWA), boolean()) -> child_specification(FWA).
...
-spec timeout(child_specification(FWD), integer()) -> child_specification(FWD).
timeout(Child, Ms) ->
...
-spec restart(child_specification(FWG), restart()) -> child_specification(FWG).
...
-spec map_data(child_specification(FWJ), fun((FWJ) -> FWL)) -> child_specification(FWL).

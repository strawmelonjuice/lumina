...
-type response(EVM) :: {response, integer(), list({binary(), binary()}), EVM}.
...
-spec set_header(response(EWB), binary(), binary()) -> response(EWB).
...
-spec prepend_header(response(EWE), binary(), binary()) -> response(EWE).
...
-spec set_body(response(any()), EWJ) -> response(EWJ).
...
-spec try_map(response(EVN), fun((EVN) -> {ok, EVP} | {error, EVQ})) -> {ok,
        response(EVP)} |
    {error, EVQ}.
...
-spec map(response(EWL), fun((EWL) -> EWN)) -> response(EWN).
...
-spec expire_cookie(response(EWW), binary(), gleam@http@cookie:attributes()) -> response(EWW).

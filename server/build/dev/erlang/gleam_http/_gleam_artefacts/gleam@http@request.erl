...
-spec set_header(request(ELS), binary(), binary()) -> request(ELS).
...
-spec prepend_header(request(ELV), binary(), binary()) -> request(ELV).
...
-spec set_body(request(any()), EMA) -> request(EMA).
set_body(Req, Body) ->
    {request,
...
-spec map(request(EMC), fun((EMC) -> EME)) -> request(EME).
...
-spec set_query(request(EMO), list({binary(), binary()})) -> request(EMO).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(EMS), gleam@http:method()) -> request(EMS).
...
-spec set_scheme(request(EMZ), gleam@http:scheme()) -> request(EMZ).
...
-spec set_host(request(ENC), binary()) -> request(ENC).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(ENF), integer()) -> request(ENF).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(ENI), binary()) -> request(ENI).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(ENL), binary(), binary()) -> request(ENL).
...
-spec remove_cookie(request(ENR), binary()) -> request(ENR).

...
-spec map_response_body(
    fun((EZI) -> gleam@http@response:response(EZJ)),
    fun((EZJ) -> EZL)
) -> fun((EZI) -> gleam@http@response:response(EZL)).
...
-spec prepend_response_header(
    fun((EZN) -> gleam@http@response:response(EZO)),
    binary(),
    binary()
) -> fun((EZN) -> gleam@http@response:response(EZO)).
...
-spec ensure_post(gleam@http@request:request(EZR)) -> {ok,
        gleam@http@request:request(EZR)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(FAA)) -> FAC)) -> fun((gleam@http@request:request(FAA)) -> FAC).

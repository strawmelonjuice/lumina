...
-type action(IVF) :: stop | {continue, IVF, fun(() -> action(IVF))}.

-opaque yielder(IVG) :: {yielder, fun(() -> action(IVG))}.

-type step(IVH, IVI) :: {next, IVH, IVI} | done.

-type chunk(IVJ, IVK) :: {another_by,
        list(IVJ),
        IVK,
        IVJ,
        fun(() -> action(IVJ))} |
...
-type sized_chunk(IVL) :: {another, list(IVL), fun(() -> action(IVL))} |
    {last, list(IVL)} |
    no_more.
...
-spec unfold_loop(IVT, fun((IVT) -> step(IVU, IVT))) -> fun(() -> action(IVU)).
...
-spec unfold(IVO, fun((IVO) -> step(IVP, IVO))) -> yielder(IVP).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> IVY)) -> yielder(IVY).
repeatedly(F) ->
...
-spec from_list(list(IWC)) -> yielder(IWC).
from_list(List) ->
...
-spec transform(yielder(IWM), IWO, fun((IWO, IWM) -> step(IWP, IWO))) -> yielder(IWP).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 204).
-spec fold_loop(fun(() -> action(IWW)), fun((IWY, IWW) -> IWY), IWY) -> IWY.
...
-spec fold(yielder(IWT), IWV, fun((IWV, IWT) -> IWV)) -> IWV.
...
-spec to_list(yielder(IXB)) -> list(IXB).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(IXE)) -> step(IXE, yielder(IXE)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(IXM)), integer()) -> fun(() -> action(IXM)).
...
-spec take(yielder(IXJ), integer()) -> yielder(IXJ).
...
-spec drop_loop(fun(() -> action(IXS)), integer()) -> action(IXS).
...
-spec map(yielder(IXV), fun((IXV) -> IXX)) -> yielder(IXX).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(IYD), yielder(IYF), fun((IYD, IYF) -> IYH)) -> yielder(IYH).
...
-spec append_loop(fun(() -> action(IYT)), fun(() -> action(IYT))) -> action(IYT).
...
-spec append(yielder(IYP), yielder(IYP)) -> yielder(IYP).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(IZB)))) -> action(IZB).
...
-spec flatten(yielder(yielder(IYX))) -> yielder(IYX).
flatten(Yielder) ->
...
-spec concat(list(yielder(IZF))) -> yielder(IZF).
concat(Yielders) ->
...
-spec flat_map(yielder(IZJ), fun((IZJ) -> yielder(IZL))) -> yielder(IZL).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(IZR)), fun((IZR) -> boolean())) -> action(IZR).
...
-spec filter(yielder(IZO), fun((IZO) -> boolean())) -> yielder(IZO).
...
-spec filter_map(yielder(IZU), fun((IZU) -> {ok, IZW} | {error, any()})) -> yielder(IZW).
...
-spec cycle(yielder(JAI)) -> yielder(JAI).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src/gleam/yielder.gleam", 709).
-spec find_loop(fun(() -> action(JAQ)), fun((JAQ) -> boolean())) -> {ok, JAQ} |
    {error, nil}.
...
-spec find(yielder(JAM), fun((JAM) -> boolean())) -> {ok, JAM} | {error, nil}.
...
-spec find_map(yielder(JAU), fun((JAU) -> {ok, JAW} | {error, any()})) -> {ok,
        JAW} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(JBN)), integer()) -> fun(() -> action({JBN,
    integer()})).
...
-spec index(yielder(JBK)) -> yielder({JBK, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(JBQ, fun((JBQ) -> JBQ)) -> yielder(JBQ).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src/gleam/yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(JBV)), fun((JBV) -> boolean())) -> fun(() -> action(JBV)).
...
-spec take_while(yielder(JBS), fun((JBS) -> boolean())) -> yielder(JBS).
...
-spec drop_while_loop(fun(() -> action(JCB)), fun((JCB) -> boolean())) -> action(JCB).
...
-spec drop_while(yielder(JBY), fun((JBY) -> boolean())) -> yielder(JBY).
...
-spec scan_loop(fun(() -> action(JCI)), fun((JCK, JCI) -> JCK), JCK) -> fun(() -> action(JCK)).
...
-spec scan(yielder(JCE), JCG, fun((JCG, JCE) -> JCG)) -> yielder(JCG).
...
-spec zip_loop(fun(() -> action(JCR)), fun(() -> action(JCT))) -> fun(() -> action({JCR,
    JCT})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(JCM), yielder(JCO)) -> yielder({JCM, JCO}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src/gleam/yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(JDG)), fun((JDG) -> JDI), JDI, list(JDG)) -> chunk(JDG, JDI).
...
-spec chunk_loop(fun(() -> action(JDB)), fun((JDB) -> JDD), JDD, JDB) -> action(list(JDB)).
...
-spec chunk(yielder(JCW), fun((JCW) -> any())) -> yielder(list(JCW)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(JDU)), integer(), list(JDU)) -> sized_chunk(JDU).
...
-spec sized_chunk_loop(fun(() -> action(JDQ)), integer()) -> fun(() -> action(list(JDQ))).
...
-spec sized_chunk(yielder(JDM), integer()) -> yielder(list(JDM)).
...
-spec intersperse_loop(fun(() -> action(JEB)), JEB) -> action(JEB).
...
-spec intersperse(yielder(JDY), JDY) -> yielder(JDY).
...
-spec any_loop(fun(() -> action(JEG)), fun((JEG) -> boolean())) -> boolean().
...
-spec any(yielder(JEE), fun((JEE) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1228).
-spec all_loop(fun(() -> action(JEK)), fun((JEK) -> boolean())) -> boolean().
...
-spec all(yielder(JEI), fun((JEI) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src/gleam/yielder.gleam", 1273).
-spec update_group_with(JFA) -> fun((gleam@option:option(list(JFA))) -> list(JFA)).
...
-spec group_updater(fun((JES) -> JET)) -> fun((gleam@dict:dict(JET, list(JES)), JES) -> gleam@dict:dict(JET, list(JES))).
group_updater(F) ->
...
-spec group(yielder(JEM), fun((JEM) -> JEO)) -> gleam@dict:dict(JEO, list(JEM)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(JFE), fun((JFE, JFE) -> JFE)) -> {ok, JFE} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(JFI)) -> {ok, JFI} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(JFQ) -> yielder(JFQ).
single(Elem) ->
    once(fun() -> Elem end).

-file("src/gleam/yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(JFW)), fun(() -> action(JFW))) -> action(JFW).
...
-spec interleave(yielder(JFS), yielder(JFS)) -> yielder(JFS).
...
-spec fold_until_loop(
    fun(() -> action(JGE)),
    fun((JGG, JGE) -> gleam@list:continue_or_stop(JGG)),
    JGG
) -> JGG.
...
-spec fold_until(
    yielder(JGA),
    JGC,
    fun((JGC, JGA) -> gleam@list:continue_or_stop(JGC))
) -> JGC.
...
-spec try_fold(yielder(JGI), JGK, fun((JGK, JGI) -> {ok, JGK} | {error, JGL})) -> {ok,
        JGK} |
    {error, JGL}.
...
-spec first(yielder(JGY)) -> {ok, JGY} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(JHC), integer()) -> {ok, JHC} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(JHK), fun((JHK) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(JHN, fun(() -> yielder(JHN))) -> yielder(JHN).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(JHQ), JHQ) -> yielder(JHQ).

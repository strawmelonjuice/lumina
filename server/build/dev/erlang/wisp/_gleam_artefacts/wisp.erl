...
-spec method_override(gleam@http@request:request(ADJR)) -> gleam@http@request:request(ADJR).
...
-spec sort_keys(list({binary(), ADLI})) -> list({binary(), ADLI}).
sort_keys(Pairs) ->
    gleam@list:sort(
        Pairs,
        fun(A, B) ->
...
-spec or_500({ok, ADKC} | {error, any()}) -> {ok, ADKC} |
    {error, gleam@http@response:response(body())}.
or_500(Result) ->
    case Result of
        {ok, Value} ->
...
-spec fn_with_bad_request_error(
    fun((ADKQ) -> {ok, ADKR} | {error, any()}),
    binary()
) -> fun((ADKQ) -> {ok, ADKR} | {error, gleam@http@response:response(body())}).
...
-spec multipart_body(
    ...
    integer(),
    integer(),
    fun((ADKK, bitstring()) -> {ok, ADKK} |
        {error, gleam@http@response:response(body())}),
    ADKK
) -> {ok, {gleam@option:option(buffered_reader()), integer(), ADKK}} |
    {error, gleam@http@response:response(body())}.

...
-spec push_key({ok, WXA} | {error, get_error()}, binary()) -> {ok, WXA} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {WZF, list(binary())}} | {error, parse_error()})
) -> {ok, {WZF, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {WZQ, list(binary())}} | {error, parse_error()},
    fun((WZQ, list(binary())) -> {ok, WZT} | {error, parse_error()})
) -> {ok, WZT} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {WZY, list(binary())}} | {error, parse_error()})
) -> {ok, {WZY, list(binary())}} | {error, parse_error()}.

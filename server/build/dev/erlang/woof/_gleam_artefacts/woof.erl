...
-spec with_context(list({binary(), binary()}), fun(() -> AERQ)) -> AERQ.
...
-spec tap_info(AERU, binary(), list({binary(), binary()})) -> AERU.
...
-spec tap_debug(AERW, binary(), list({binary(), binary()})) -> AERW.
...
-spec tap_warning(AERY, binary(), list({binary(), binary()})) -> AERY.
...
-spec tap_error(AESA, binary(), list({binary(), binary()})) -> AESA.
...
-spec time(binary(), fun(() -> AESJ)) -> AESJ.
time(Label, Body) ->

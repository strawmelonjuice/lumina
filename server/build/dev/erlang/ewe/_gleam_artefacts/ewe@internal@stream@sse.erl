...
-type s_s_e_next(ORF) :: {continue, ORF} |
    normal_stop |
    {abnormal_stop, binary()}.

-type s_s_e_messages(ORG) :: {user, ORG} | close.
...
?DOC(false).
-spec create_socket_selector(gleam@erlang@process:subject(ORQ)) -> gleam@erlang@process:selector(s_s_e_messages(ORQ)).
...
-spec start(
    glisten@transport:transport(),
    glisten@socket:socket(),
    fun((gleam@erlang@process:subject(ORJ)) -> ORL),
    fun((s_s_e_connection(), ORL, ORJ) -> s_s_e_next(ORL)),
    fun((s_s_e_connection(), ORL) -> nil)

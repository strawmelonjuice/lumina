...
-type websocket_message(OVF) :: {frame, websocks:frame()} | {user_message, OVF}.

-type websocket_next(OVG, OVH) :: {continue,
        OVG,
        gleam@option:option(gleam@erlang@process:selector(OVH))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type websocket_state(OVI) :: {websocket_state, OVI, websocks:context()}.

-type internal_message(OVJ) :: {packet, bitstring()} |
    close |
    tcp_passive |
    {user, OVJ} |
    invalid.

-type resolve_state(OVK, OVL) :: {resolve_state,
        glisten@socket:socket(),
        glisten@transport:transport(),
        fun((websocket_connection(), OVK, websocket_message(OVL)) -> websocket_next(OVK, OVL)),
        websocket_next(OVK, internal_message(OVL))}.
...
?DOC(false).
-spec handle_close(
    fun((websocket_connection(), OXN) -> nil),
    websocket_state(OXN),
    websocket_connection(),
    gleam@option:option(binary())
) -> gleam@otp@actor:next(websocket_state(OXN), internal_message(any())).
...
-spec handle_user_message(
    ...
    websocket_state(OXF),
    OXH,
    fun((websocket_connection(), OXF, websocket_message(OXH)) -> websocket_next(OXF, OXH)),
    fun((websocket_connection(), OXF) -> nil)
) -> gleam@otp@actor:next(websocket_state(OXF), internal_message(OXH)).
...
?DOC(false).
-spec handle_frame(
    resolve_state(OWY, OWZ),
    websocks:context(),
    websocks:frame()
) -> websocks:resolve_next(resolve_state(OWY, OWZ)).
...
-spec handle_valid_packet(
    ...
    websocket_state(OWQ),
    bitstring(),
    fun((websocket_connection(), OWQ, websocket_message(OWS)) -> websocket_next(OWQ, OWS)),
    fun((websocket_connection(), OWQ) -> nil)
) -> gleam@otp@actor:next(websocket_state(OWQ), internal_message(OWS)).
...
-spec start(
    glisten@transport:transport(),
    glisten@socket:socket(),
    fun((websocket_connection(), gleam@erlang@process:selector(OWD)) -> {OWC,
        gleam@erlang@process:selector(OWD)}),
    fun((websocket_connection(), OWC, websocket_message(OWD)) -> websocket_next(OWC, OWD)),
    fun((websocket_connection(), OWC) -> nil),
    list(binary()),
    boolean()

...
-type message(LEL) :: {internal, internal_message()} | {user, LEL}.

-type loop_message(LEM) :: {packet, bitstring()} | {custom, LEM}.

-type loop_state(LEN, LEO) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(LEO)),
        glisten@transport:transport(),
        LEN}.

-type connection(LEP) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(LEP))}.

-type next(LEQ, LER) :: {continue,
        LEQ,
        gleam@option:option(gleam@erlang@process:selector(LER))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(LES, LET) :: {handler,
        glisten@socket:socket(),
        fun((LES, loop_message(LET), connection(LET)) -> next(LES, loop_message(LET))),
        fun((connection(LET)) -> {LES,
            gleam@option:option(gleam@erlang@process:selector(LET))}),
        gleam@option:option(fun((LES) -> nil)),
        glisten@transport:transport(),
        glisten@socket@options:active_state()}.

-file("src/glisten/internal/handler.gleam", 65).
?DOC(false).
-spec continue(LFG) -> next(LFG, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten/internal/handler.gleam", 69).
?DOC(false).
-spec with_selector(next(LFK, LFL), gleam@erlang@process:selector(LFL)) -> next(LFK, LFL).
...
?DOC(false).
-spec start(handler(any(), LGA)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(LGA)))} |

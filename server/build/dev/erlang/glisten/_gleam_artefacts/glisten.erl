...
-type message(MBZ) :: {packet, bitstring()} | {user, MBZ}.
...
-type connection(MCA) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(MCA))}.

-opaque next(MCB, MCC) :: {continue,
        MCB,
        gleam@option:option(gleam@erlang@process:selector(MCC))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(MCD, MCE) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(MCE)) -> {MCD,
            gleam@option:option(gleam@erlang@process:selector(MCE))}),
        fun((MCD, message(MCE), connection(MCE)) -> next(MCD, message(MCE))),
        gleam@option:option(fun((MCD) -> nil)),
        integer(),
        boolean(),
        boolean(),
        ...
        gleam@option:option(gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(MCE))))),
...
-spec continue(MDE) -> next(MDE, any()).
continue(State) ->
    {continue, State, none}.

-file("src/glisten.gleam", 152).
-spec with_selector(next(MDI, MDJ), gleam@erlang@process:selector(MDJ)) -> next(MDI, MDJ).
...
?DOC(false).
-spec convert_next(next(MDX, MDY)) -> glisten@internal@handler:next(MDX, MDY).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(MED, MEE), fun((MEE) -> MEH)) -> next(MED, MEH).
...
-spec map_user_selector(gleam@erlang@process:selector(message(MEK))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(MEK)).
...
-spec convert_loop(
    fun((MEP, message(MEQ), connection(MEQ)) -> next(MEP, message(MEQ)))
) -> fun((MEP, glisten@internal@handler:loop_message(MEQ), glisten@internal@handler:connection(MEQ)) -> glisten@internal@handler:next(MEP, glisten@internal@handler:loop_message(MEQ))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(MEV)) -> {MEX,
        gleam@option:option(gleam@erlang@process:selector(MEV))})
) -> fun((glisten@internal@handler:connection(MEV)) -> {MEX,
    gleam@option:option(gleam@erlang@process:selector(MEV))}).
...
-spec new(
    fun((connection(MFD)) -> {MFF,
        gleam@option:option(gleam@erlang@process:selector(MFD))}),
    fun((MFF, message(MFD), connection(MFD)) -> next(MFF, message(MFD)))
) -> builder(MFF, MFD).
new(On_init, Loop) ->
    {builder,
        loopback,
...
-spec with_close(builder(MFM, MFN), fun((MFM) -> nil)) -> builder(MFM, MFN).
...
-spec with_pool_size(builder(MFS, MFT), integer()) -> builder(MFS, MFT).
...
?DOC(false).
-spec with_http2(builder(MFY, MFZ)) -> builder(MFY, MFZ).
with_http2(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_ipv6(builder(MGK, MGL)) -> builder(MGK, MGL).
with_ipv6(Builder) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec with_tls(builder(MGQ, MGR), binary(), binary()) -> builder(MGQ, MGR).
...
-spec with_connection_factory_name(
    builder(MHJ, MHK),
    gleam@erlang@process:name(gleam@otp@factory_supervisor:message(glisten@socket:socket(), gleam@erlang@process:subject(glisten@internal@handler:message(MHK))))
) -> builder(MHJ, MHK).
...
-spec bind(builder(MGE, MGF), binary()) -> builder(MGE, MGF).

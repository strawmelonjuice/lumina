...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(QGW)) -> {ok, QGW} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(QGQ)) -> {ok, QGQ} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(QGM)) -> {ok, QGM} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(QHC), fun((QHC) -> json())) -> json().
...
-spec array(list(QHG), fun((QHG) -> json())) -> json().

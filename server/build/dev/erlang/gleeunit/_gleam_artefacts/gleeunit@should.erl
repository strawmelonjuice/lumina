...
-spec be_ok({ok, RBY} | {error, any()}) -> RBY.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, RCD}) -> RCD.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(RCG)) -> RCG.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;

...
-opaque subject(DPQ) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DPQ)}.

-type name(DPR) :: any() | {gleam_phantom, DPR}.

-type do_not_leak() :: any().

-type selector(DPS) :: any() | {gleam_phantom, DPS}.
...
-spec 'receive'(subject(DQQ), integer()) -> {ok, DQQ} | {error, nil}.
...
-spec receive_forever(subject(DQY)) -> DQY.
...
-spec selector_receive(selector(DRC), integer()) -> {ok, DRC} | {error, nil}.
...
-spec selector_receive_forever(selector(DRG)) -> DRG.
...
-spec map_selector(selector(DRI), fun((DRI) -> DRK)) -> selector(DRK).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRM), selector(DRM)) -> selector(DRM).
merge_selector(A, B) ->
...
-spec select_map(selector(DRX), subject(DRZ), fun((DRZ) -> DRX)) -> selector(DRX).
...
-spec select(selector(DRT), subject(DRT)) -> selector(DRT).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSH)
) -> selector(DSH).
...
-spec select_other(selector(DSL), fun((gleam@dynamic:dynamic_()) -> DSL)) -> selector(DSL).
...
-spec deselect(selector(DSC), subject(any())) -> selector(DSC).
...
-spec select_specific_monitor(selector(DSX), monitor(), fun((down()) -> DSX)) -> selector(DSX).
...
-spec select_monitors(selector(DTA), fun((down()) -> DTA)) -> selector(DTA).
...
-spec select_trapped_exits(selector(DRQ), fun((exit_message()) -> DRQ)) -> selector(DRQ).
...
-spec deselect_specific_monitor(selector(DTD), monitor()) -> selector(DTD).
...
-spec send_after(subject(DTZ), integer(), DTZ) -> timer().
...
-spec perform_call(
    subject(DTG),
    fun((subject(DTI)) -> DTG),
    fun((selector(DTI)) -> {ok, DTI} | {error, nil})
) -> DTI.
...
-spec call(subject(DTN), integer(), fun((subject(DTP)) -> DTN)) -> DTP.
...
-spec call_forever(subject(DTR), fun((subject(DTT)) -> DTR)) -> DTT.

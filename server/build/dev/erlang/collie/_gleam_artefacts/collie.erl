...
-opaque next(HMZ, HNA) :: {continue,
        HMZ,
        gleam@option:option(gleam@erlang@process:selector(HNA))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque initialised(HNB, HNC) :: {initialised,
        HNB,
        gleam@option:option(gleam@erlang@process:selector(HNC))}.

-type message(HND) :: {text, binary()} | {binary, bitstring()} | {user, HND}.
...
-opaque builder(HNE, HNF, HNG) :: {builder,
        gleam@http@request:request(HNE),
        gleam@option:option(gleam@erlang@process:name(websocket_message(HNG))),
        integer(),
        fun((gleam@erlang@process:subject(websocket_message(HNG))) -> {ok,
                initialised(HNF, HNG)} |
            {error, binary()}),
        fun((connection(), HNF, message(HNG)) -> next(HNF, HNG)),
        fun((HNF, close_reason()) -> nil)}.

-type websocket_state(HNH, HNI) :: {websocket_state,
        connection(),
        HNH,
        websocks:context(),
        fun((connection(), HNH, message(HNI)) -> next(HNH, HNI)),
        fun((HNH, close_reason()) -> nil)}.

-opaque websocket_message(HNJ) :: {packet, bitstring()} |
    {user_message, HNJ} |
    passive |
...
-spec continue_with_selector(HNQ, gleam@erlang@process:selector(HNR)) -> next(HNQ, HNR).
...
-spec initialised(HOD) -> initialised(HOD, any()).
initialised(State) ->
    {initialised, State, none}.

-file("src/collie.gleam", 170).
?DOC(" Adds a selector to receive messages with.\n").
-spec selecting(initialised(HOH, any()), gleam@erlang@process:selector(HOL)) -> initialised(HOH, HOL).
...
-spec new(gleam@http@request:request(HOP), HOR) -> builder(HOP, HOR, any()).
new(Request, State) ->
    {builder,
        Request,
        none,
...
-spec new_with_initialiser(
    gleam@http@request:request(HOW),
    fun((gleam@erlang@process:subject(websocket_message(HOY))) -> {ok,
            initialised(HPB, HOY)} |
        {error, binary()})
) -> builder(HOW, HPB, HOY).
...
-spec with_connection_timeout(builder(HPJ, HPK, HPL), integer()) -> builder(HPJ, HPK, HPL).
...
-spec named(
    builder(HPS, HPT, HPU),
    gleam@erlang@process:name(websocket_message(HPU))
) -> builder(HPS, HPT, HPU).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
...
-spec on_message(
    builder(HQD, HQE, HQF),
    fun((connection(), HQE, message(HQF)) -> next(HQE, HQF))
) -> builder(HQD, HQE, HQF).
...
-spec on_close(builder(HQP, HQQ, HQR), fun((HQQ, close_reason()) -> nil)) -> builder(HQP, HQQ, HQR).
...
-spec to_user_message(HQY) -> websocket_message(HQY).
to_user_message(Message) ->
    {user_message, Message}.

-file("src/collie.gleam", 559).
-spec create_socket_selector(
    gleam@erlang@process:subject(websocket_message(HRA)),
    gleam@option:option(gleam@erlang@process:selector(HRA))
) -> gleam@erlang@process:selector(websocket_message(HRA)).
...
-spec unwrap_socket(
    {ok, HSL} | {error, collie@internal@socket:socket_reason()},
    fun((HSL) -> {ok, HSO} | {error, binary()})
) -> {ok, HSO} | {error, binary()}.
...
-spec new_resolve_state(websocket_state(HTD, HTE)) -> resolve_state(HTD, HTE).
...
-spec resolve_next(resolve_state(HTR, HTS), websocket_state(HTR, HTS)) -> gleam@otp@actor:next(websocket_state(HTR, HTS), websocket_message(HTS)).
...
-spec call_handler(resolve_state(HUP, HUQ), message(HUQ)) -> websocks:resolve_next(resolve_state(HUP, HUQ)).
...
-spec handle_frame(
    resolve_state(HUI, HUJ),
    websocks:context(),
    websocks:frame()
) -> websocks:resolve_next(resolve_state(HUI, HUJ)).
...
-spec handle_packet(bitstring(), websocket_state(HTI, HTJ)) -> gleam@otp@actor:next(websocket_state(HTI, HTJ), websocket_message(HTJ)).
...
-spec handle_message(websocket_state(HST, HSU), websocket_message(HSU)) -> gleam@otp@actor:next(websocket_state(HST, HSU), websocket_message(HSU)).
...
-spec start(builder(any(), any(), HRL)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(HRL)))} |
...
-spec supervised(builder(any(), any(), HRW)) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(websocket_message(HRW))).
supervised(Builder) ->

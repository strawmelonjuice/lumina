...
-type array(QVL) :: any() | {gleam_phantom, QVL}.
...
-spec from_list(list(QVO)) -> array(QVO).
from_list(List) ->
    erlang:list_to_tuple(List).

-file("src/glearray.gleam", 44).
?DOC(" Converts an array to a list.\n").
-spec to_list(array(QVR)) -> list(QVR).
to_list(Array) ->
...
-spec get(array(QVW), integer()) -> {ok, QVW} | {error, nil}.
get(Array, Index) ->
...
-spec get_or_default(array(QWA), integer(), QWA) -> QWA.
...
-spec copy_set(array(QWE), integer(), QWE) -> {ok, array(QWE)} | {error, nil}.
...
-spec copy_push(array(QWO), QWO) -> array(QWO).
...
-spec copy_insert(array(QWR), integer(), QWR) -> {ok, array(QWR)} | {error, nil}.

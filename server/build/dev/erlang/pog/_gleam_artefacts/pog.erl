...
-type transaction_error(ABSN) :: {transaction_query_error, query_error()} |
    {transaction_rolled_back, ABSN}.

-type returned(ABSO) :: {returned, integer(), list(ABSO)}.
...
-opaque 'query'(ABSP) :: {'query',
        binary(),
        list(value()),
        gleam@dynamic@decode:decoder(ABSP),
        integer()}.
...
-spec array(fun((ABTG) -> value()), list(ABTG)) -> value().
...
-spec nullable(fun((ABUB) -> value()), gleam@option:option(ABUB)) -> value().
...
-spec transaction_layer(
    single_connection(),
    fun((connection()) -> {ok, ABTR} | {error, ABTS})
) -> {ok, ABTR} | {error, transaction_error(ABTS)}.
...
-spec transaction(
    connection(),
    fun((connection()) -> {ok, ABTK} | {error, ABTL})
) -> {ok, ABTK} | {error, transaction_error(ABTL)}.
...
-spec returning('query'(any()), gleam@dynamic@decode:decoder(ABUN)) -> 'query'(ABUN).
...
-spec parameter('query'(ABUQ), value()) -> 'query'(ABUQ).
...
-spec timeout('query'(ABUT), integer()) -> 'query'(ABUT).
...
-spec execute('query'(ABUW), connection()) -> {ok, returned(ABUW)} |
    {error, query_error()}.
execute(Query, Pool) ->

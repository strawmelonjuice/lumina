...
function status_to_bit_array(status) {
  if (status === 100) {
    ...
    return toBitArray([stringBits("Multi-Status")]);
  } else if (status === 208) {
    return toBitArray([stringBits("Already Reported")]);
  } else if (status === 226) {
    return toBitArray([stringBits("IM Used")]);
  } else if (status === 300) {
    ...
    return toBitArray([stringBits("Misdirected Request")]);
  } else if (status === 422) {
    return toBitArray([stringBits("Unprocessable Entity")]);
  } else if (status === 423) {
    return toBitArray([stringBits("Locked")]);
  } else if (status === 424) {
    return toBitArray([stringBits("Failed Dependency")]);
  } else if (status === 425) {
 ...
 * @ignore
 */
function encode_status_line(status) {
  let status_name = status_to_bit_array(status);
  let status$1 = $int.to_string(status);
  return toBitArray([
    stringBits("HTTP/1.1 "),
    stringBits(status$1),
    stringBits(" "),

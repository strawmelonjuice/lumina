...
-opaque next(HME, HMF) :: {continue,
        HME,
        gleam@option:option(gleam@erlang@process:selector(HMF))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque initialised(HMG, HMH) :: {initialised,
        HMG,
        gleam@option:option(gleam@erlang@process:selector(HMH))}.

-type message(HMI) :: {text, binary()} | {binary, bitstring()} | {user, HMI}.
...
-opaque builder(HMJ, HMK, HML) :: {builder,
        gleam@http@request:request(HMJ),
        gleam@option:option(gleam@erlang@process:name(websocket_message(HML))),
        integer(),
        fun((gleam@erlang@process:subject(websocket_message(HML))) -> {ok,
                initialised(HMK, HML)} |
            {error, binary()}),
        fun((connection(), HMK, message(HML)) -> next(HMK, HML)),
        fun((HMK, close_reason()) -> nil)}.

-type websocket_state(HMM, HMN) :: {websocket_state,
        connection(),
        HMM,
        websocks:context(),
...
-opaque websocket_message(HMO) :: {packet, bitstring()} |
    {user_message, HMO} |
    passive |
...
-spec continue_with_selector(HMV, gleam@erlang@process:selector(HMW)) -> next(HMV, HMW).
...
-spec initialised(HNI) -> initialised(HNI, any()).
initialised(State) ->
    {initialised, State, none}.

-file("src/collie.gleam", 170).
?DOC(" Adds a selector to receive messages with.\n").
-spec selecting(initialised(HNM, any()), gleam@erlang@process:selector(HNQ)) -> initialised(HNM, HNQ).
...
-spec new(gleam@http@request:request(HNU), HNW) -> builder(HNU, HNW, any()).
new(Request, State) ->
    {builder,
        Request,
        none,
...
-spec new_with_initialiser(
    gleam@http@request:request(HOB),
    fun((gleam@erlang@process:subject(websocket_message(HOD))) -> {ok,
            initialised(HOG, HOD)} |
        {error, binary()})
) -> builder(HOB, HOG, HOD).
...
-spec with_connection_timeout(builder(HOO, HOP, HOQ), integer()) -> builder(HOO, HOP, HOQ).
...
-spec named(
    builder(HOX, HOY, HOZ),
    gleam@erlang@process:name(websocket_message(HOZ))
) -> builder(HOX, HOY, HOZ).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
...
-spec on_message(
    builder(HPI, HPJ, HPK),
    fun((connection(), HPJ, message(HPK)) -> next(HPJ, HPK))
) -> builder(HPI, HPJ, HPK).
...
-spec on_close(builder(HPU, HPV, HPW), fun((HPV, close_reason()) -> nil)) -> builder(HPU, HPV, HPW).
...
-spec to_user_message(HQD) -> websocket_message(HQD).
to_user_message(Message) ->
    {user_message, Message}.

-file("src/collie.gleam", 559).
-spec create_socket_selector(
    gleam@erlang@process:subject(websocket_message(HQF)),
    gleam@option:option(gleam@erlang@process:selector(HQF))
) -> gleam@erlang@process:selector(websocket_message(HQF)).
...
-spec unwrap_socket(
    {ok, HRQ} | {error, collie@internal@socket:socket_reason()},
    fun((HRQ) -> {ok, HRT} | {error, binary()})
) -> {ok, HRT} | {error, binary()}.
...
-spec new_resolve_state(websocket_state(HSI, HSJ)) -> resolve_state(HSI, HSJ).
...
-spec resolve_next(resolve_state(HSW, HSX), websocket_state(HSW, HSX)) -> gleam@otp@actor:next(websocket_state(HSW, HSX), websocket_message(HSX)).
...
-spec call_handler(resolve_state(HTU, HTV), message(HTV)) -> websocks:resolve_next(resolve_state(HTU, HTV)).
...
-spec handle_frame(
    resolve_state(HTN, HTO),
    websocks:context(),
    websocks:frame()
) -> websocks:resolve_next(resolve_state(HTN, HTO)).
...
-spec handle_packet(bitstring(), websocket_state(HSN, HSO)) -> gleam@otp@actor:next(websocket_state(HSN, HSO), websocket_message(HSO)).
...
-spec handle_message(websocket_state(HRY, HRZ), websocket_message(HRZ)) -> gleam@otp@actor:next(websocket_state(HRY, HRZ), websocket_message(HRZ)).
...
-spec start(builder(any(), any(), HQQ)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(HQQ)))} |
...
-spec supervised(builder(any(), any(), HRB)) -> gleam@otp@supervision:child_specification(gleam@erlang@process:subject(websocket_message(HRB))).
supervised(Builder) ->

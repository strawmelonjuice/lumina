...
-spec push_key({ok, DZP} | {error, get_error()}, binary()) -> {ok, DZP} |
    {error, get_error()}.
push_key(Result, Key) ->
    case Result of
        {ok, T} ->
...
-spec expect_end_of_line(
    list(binary()),
    fun((list(binary())) -> {ok, {EBU, list(binary())}} | {error, parse_error()})
) -> {ok, {EBU, list(binary())}} | {error, parse_error()}.
...
-spec do(
    {ok, {ECF, list(binary())}} | {error, parse_error()},
    fun((ECF, list(binary())) -> {ok, ECI} | {error, parse_error()})
) -> {ok, ECI} | {error, parse_error()}.
do(Result, Next) ->
    case Result of
        {ok, {A, Input}} ->
            Next(A, Input);
...
-spec expect(
    list(binary()),
    binary(),
    fun((list(binary())) -> {ok, {ECN, list(binary())}} | {error, parse_error()})
) -> {ok, {ECN, list(binary())}} | {error, parse_error()}.

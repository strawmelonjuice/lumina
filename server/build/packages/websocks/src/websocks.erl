...
-type resolve_next(DXP) :: {continue, DXP} | {stop, DXP}.
...
-spec resolve_and_handle_single_frame(
    ...
    context(),
    DYZ,
    fun((DYZ, context(), frame()) -> resolve_next(DYZ))
) -> {ok, {resolve_next(DYZ), context()}} | {error, resolve_error()}.
...
-spec do_process_incoming_frames(
    ...
    context(),
    DYW,
    fun((DYW, context(), frame()) -> resolve_next(DYW))
) -> {ok, {DYW, context()}} | {error, process_error()}.
...
-spec process_incoming_frames(
    ...
    context(),
    DYT,
    fun((DYT, context(), frame()) -> resolve_next(DYT))
) -> {ok, {DYT, context()}} | {error, process_error()}.

...
-opaque chunked_next(NAD) :: {chunked_continue, NAD} |
    chunked_stop |
    {chunked_abnormal_stop, binary()}.

-opaque websocket_next(NAE, NAF) :: {websocket_continue,
        NAE,
        gleam@option:option(gleam@erlang@process:selector(NAF))} |
    websocket_normal_stop |
    {websocket_abnormal_stop, binary()}.

-type websocket_message(NAG) :: {text, binary()} |
    {binary, bitstring()} |
    {user, NAG}.
...
-opaque s_s_e_next(NAH) :: {s_s_e_continue, NAH} |
    s_s_e_normal_stop |
...
-spec chunked_continue(NBG) -> chunked_next(NBG).
...
-spec to_internal_chunked_next(chunked_next(NBM)) -> ewe@internal@stream@chunked:chunked_next(NBM).
...
-spec chunked_body(
    ...
    fun((gleam@erlang@process:subject(NBR)) -> NBT),
    fun((ewe@internal@stream@chunked:chunked_body(), NBT, NBR) -> chunked_next(NBT)),
    fun((ewe@internal@stream@chunked:chunked_body(), NBT) -> nil)
) -> gleam@http@response:response(response_body()).
...
-spec websocket_continue(NBX) -> websocket_next(NBX, any()).
...
-spec websocket_continue_with_selector(NCB, gleam@erlang@process:selector(NCC)) -> websocket_next(NCB, NCC).
...
-spec to_websocket_next(ewe@internal@stream@websocket:websocket_next(NCO, NCP)) -> websocket_next(NCO, NCP).
...
-spec to_internal_websocket_next(websocket_next(NCU, NCV)) -> ewe@internal@stream@websocket:websocket_next(NCU, NCV).
...
-spec transform_websocket_message(
    ewe@internal@stream@websocket:websocket_message(NDA)
) -> {ok, websocket_message(NDA)} | {error, nil}.
...
-spec upgrade_websocket(
    gleam@http@request:request(ewe@internal@http1:connection()),
    fun((ewe@internal@stream@websocket:websocket_connection(), gleam@erlang@process:selector(NDF)) -> {NDH,
        gleam@erlang@process:selector(NDF)}),
    fun((ewe@internal@stream@websocket:websocket_connection(), NDH, websocket_message(NDF)) -> websocket_next(NDH, NDF)),
    fun((ewe@internal@stream@websocket:websocket_connection(), NDH) -> nil)
) -> gleam@http@response:response(response_body()).
...
-spec sse_continue(NDU) -> s_s_e_next(NDU).
...
-spec sse(
    gleam@http@request:request(ewe@internal@http1:connection()),
    fun((gleam@erlang@process:subject(NED)) -> NEF),
    fun((ewe@internal@stream@sse:s_s_e_connection(), NEF, NED) -> s_s_e_next(NEF)),
    fun((ewe@internal@stream@sse:s_s_e_connection(), NEF) -> nil)
) -> gleam@http@response:response(response_body()).
...
-spec to_internal_sse_next(s_s_e_next(NEA)) -> ewe@internal@stream@sse:s_s_e_next(NEA).

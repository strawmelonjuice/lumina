...
-type websocket_message(MJG) :: {frame, websocks:frame()} | {user_message, MJG}.

-type websocket_next(MJH, MJI) :: {continue,
        MJH,
        gleam@option:option(gleam@erlang@process:selector(MJI))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type websocket_state(MJJ) :: {websocket_state, MJJ, websocks:context()}.

-type internal_message(MJK) :: {packet, bitstring()} |
    close |
    tcp_passive |
    {user, MJK} |
    invalid.

-type resolve_state(MJL, MJM) :: {resolve_state,
        glisten@socket:socket(),
        glisten@transport:transport(),
        fun((websocket_connection(), MJL, websocket_message(MJM)) -> websocket_next(MJL, MJM)),
        websocket_next(MJL, internal_message(MJM))}.
...
?DOC(false).
-spec handle_close(
    fun((websocket_connection(), MLO) -> nil),
    websocket_state(MLO),
    websocket_connection(),
    gleam@option:option(binary())
) -> gleam@otp@actor:next(websocket_state(MLO), internal_message(any())).
...
-spec handle_user_message(
    ...
    websocket_state(MLG),
    MLI,
    fun((websocket_connection(), MLG, websocket_message(MLI)) -> websocket_next(MLG, MLI)),
    fun((websocket_connection(), MLG) -> nil)
) -> gleam@otp@actor:next(websocket_state(MLG), internal_message(MLI)).
...
?DOC(false).
-spec handle_frame(
    resolve_state(MKZ, MLA),
    websocks:context(),
    websocks:frame()
) -> websocks:resolve_next(resolve_state(MKZ, MLA)).
...
-spec handle_valid_packet(
    ...
    websocket_state(MKR),
    bitstring(),
    fun((websocket_connection(), MKR, websocket_message(MKT)) -> websocket_next(MKR, MKT)),
    fun((websocket_connection(), MKR) -> nil)
) -> gleam@otp@actor:next(websocket_state(MKR), internal_message(MKT)).
...
-spec start(
    glisten@transport:transport(),
    glisten@socket:socket(),
    fun((websocket_connection(), gleam@erlang@process:selector(MKE)) -> {MKD,
        gleam@erlang@process:selector(MKE)}),
    fun((websocket_connection(), MKD, websocket_message(MKE)) -> websocket_next(MKD, MKE)),
    fun((websocket_connection(), MKD) -> nil),
    list(binary()),
    boolean()

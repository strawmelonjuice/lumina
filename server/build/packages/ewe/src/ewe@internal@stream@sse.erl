...
-type s_s_e_next(MFG) :: {continue, MFG} |
    normal_stop |
    {abnormal_stop, binary()}.

-type s_s_e_messages(MFH) :: {user, MFH} | close.
...
?DOC(false).
-spec create_socket_selector(gleam@erlang@process:subject(MFR)) -> gleam@erlang@process:selector(s_s_e_messages(MFR)).
...
-spec start(
    glisten@transport:transport(),
    glisten@socket:socket(),
    fun((gleam@erlang@process:subject(MFK)) -> MFM),
    fun((s_s_e_connection(), MFM, MFK) -> s_s_e_next(MFM)),
    fun((s_s_e_connection(), MFM) -> nil)

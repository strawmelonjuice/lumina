...
export const siteHome = () => {
	 ...
	 */
	function switchpages(toPageName: string) {
		let to = toPageName;
		if (toPageName === "") to = "home";
		for (let key in subPageList) {
	...
	document
		.getElementById("mobiletimelineswitcher")
		.setAttribute("onclick", showMobiletimelineSwitcher.name);
	...
	setInterval(() => {
		for (const e of <HTMLCollectionOf<HTMLElement>>(
			...
			const d = new Date(parseInt(e.innerText) * 1000);

...
pub fn encode_ws_msg_from_server(ws_msg_from_server: WsMsgFromServer) -> json.Json {
	case ws_msg_from_server {
		Greeting(greeting:) -> json.object([
		#("type", json.string("greeting")),
		#("greeting", json.string(greeting)),
		])
		RegisterPrecheckResponse(ok:, why:) -> json.object([
		#("type", json.string("register_precheck_response")),
		#("ok", json.bool(ok)),
		#("why", json.string(why)),
		])
		AuthenticationSuccess(username:, token:) -> json.object([
		#("type", json.string("authentication_success")),
		#("username", json.string(username)),
		#("token", json.string(token)),
		])
		AuthenticationFailure -> json.object([
		#("type", json.string("authentication_failure")),
		])
		TimeLineResponse(timeline_name:, timeline_id:, items:, total_count:, page:, has_more:) -> json.object([
		...
		OwnUserInformationResponse(username:, email:, avatar:, uuid:, unread_notifications:) -> json.object([
		#("type", json.string("own_user_information_response")),
		#("username", json.string(username)),
		#("email", json.string(email)),
		#("avatar", case avatar {
			None -> json.null()
			Some(value) -> json.preprocessed_array([
			json.string(value.0),
			json.string(value.1),
			])
		}),
...
pub type WsMsgFromClient {
	Introduction(client_kind: String, try_revive: option.Option(String))
...
pub fn encode_ws_msg_from_client(message: WsMsgFromClient) -> json.Json {
	case message {
		Introduction(client_kind:, try_revive:) -> json.object([
		#("type", json.string("introduction")),
		#("client_kind", json.string(client_kind)),
...
pub fn ws_msg_from_client_decoder() -> decode.Decoder(WsMsgFromClient) {
	...
	case variant {
		"introduction" -> {
			use client_kind <- decode.field("client_kind", decode.string)

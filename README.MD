...
> Notice:
> This repo is edited on [CodeBerg](https://codeberg.org/strawmelonjuice/lumina).
> Any changes in the code on github will be overwritten by automated force-push from Codeberg.
...
During development, I use the following watchexec command that just truly checks everything and then keeps rebuilding.

```sh
watchexec -e rs,gleam,toml,css,ts,json -- cargo run
```
